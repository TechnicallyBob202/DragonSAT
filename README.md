...
💖 **Like what you see?** If DragonSAT helps you or your students, consider supporting its development — hit the **Sponsor** button on the right side of this repo!
...
<p align="center">
  <img src="https://github.com/user-attachments/assets/77fe1e8c-e6fc-40d5-a974-89ad8cbe059f" alt="DragonSAT Dashboard" width="700" />
</p>

<p align="center">
  <img src="https://github.com/user-attachments/assets/da9be0a0-1b5a-49e2-ad92-494284e38961" alt="DragonSAT Login" width="350" />
</p>
...
- **Admin Console** — User management for admins: list users, delete accounts, reset passwords, promote/demote roles. The first registered user is automatically granted the admin role
...
│   └── src/
│       ├── routes/        # /api/questions, /api/progress, /api/auth, /api/admin
...
| `POST` | `/api/auth/link-google` | Link Google to existing account (JWT required) |
...
| `GET` | `/api/progress/analytics` | Get per-domain accuracy breakdown (JWT required) |
...
| `PATCH` | `/api/admin/users/:id/role` | Change user role (`admin` or `user`) |

> **First user = admin:** The very first account registered on a fresh database is automatically granted the `admin` role. Additional admins can be promoted from the Admin Console.
...
**Backend:** Express.js, SQLite (sqlite3), TypeScript, Axios, JWT (jsonwebtoken)
