...
export function createAuthRouter(db: sqlite3.Database): Router {
  ...
  router.post('/register', async (req: Request, res: Response) => {
    ...
    try {
      ...
      await runAsync(
        ...
        [id, name.trim(), username, email.toLowerCase(), password_hash, role]
      );

      const token = jwt.sign({ userId: id, role }, JWT_SECRET, { expiresIn: '7d' });
      res.json({ success: true, token, user: { id, username, name: name.trim(), email: email.toLowerCase(), role } });
    } catch (err) {
  ...
  router.post('/login', async (req: Request, res: Response) => {
    ...
    try {
      ...
      res.json({ success: true, token, user: { id: user.id, username: user.username, name: user.name, email: user.email ?? null, role: user.role ?? 'user' } });
    } catch (err) {
  ...
  router.post('/google', async (req: Request, res: Response) => {
    ...
    try {
      ...
      let user = await getAsync(db, 'SELECT id, username, name, email, role FROM users WHERE google_id = ?', [googleId]);

      if (!user && email) {
        // Link to existing account that has the same email
        user = await getAsync(db, 'SELECT id, username, name, email, role FROM users WHERE email = ?', [email.toLowerCase()]);
        if (user) {
      ...
      if (!user) {
        ...
        await runAsync(
          db,
          'INSERT INTO users (id, name, username, email, google_id, role) VALUES (?, ?, ?, ?, ?, ?)',
          [id, name ?? username, username, email ? email.toLowerCase() : null, googleId, role]
        );
        user = { id, username, name: name ?? username, email: email ? email.toLowerCase() : null, role };
      }
  ...
  router.get('/me', requireAuth, async (req: AuthRequest, res: Response) => {
    try {
      const user = await getAsync(
        db,
        'SELECT id, name, username, email, google_id, role FROM users WHERE id = ?',
        [req.userId]
      );
      if (!user) {
      ...
      res.json({ success: true, user: { id: user.id, name: user.name, username: user.username, email: user.email ?? null, googleLinked: !!user.google_id, role: user.role ?? 'user' } });
    } catch (err) {
